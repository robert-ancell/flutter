...
static constexpr char kOnWindowChangedMethod[] = "onWindowChanged";
...
static constexpr char kRelativePositionKey[] = "relativePosition";
...
void fl_windowing_channel_on_window_changed(FlWindowingChannel* self,
                                            int64_t view_id,
                                            FlWindowingSize* size,
                                            FlWindowingSize* relative_position,
  ...
  if (size != nullptr) {
    fl_value_set_string_take(args, kSizeKey, make_size_value(size));
  }
  if (relative_position != nullptr) {
    fl_value_set_string_take(args, kRelativePositionKey,
                             make_size_value(relative_position));
  }
  fl_method_channel_invoke_method(self->channel, kOnWindowChangedMethod, args,
                                  cancellable, callback, user_data);
}

gboolean fl_windowing_channel_on_window_changed_finish(GObject* object,
                                                       GAsyncResult* result,
                                                       GError** error) {
