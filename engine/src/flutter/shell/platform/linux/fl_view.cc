...
static void fl_view_make_current(FlRenderable* renderable) {
  ...
  fl_subsurface_make_current(self->render_area);
}

// Implements FlRenderable::swap_buffers
static void fl_view_swap_buffers(FlRenderable* renderable) {
  FlView* self = FL_VIEW(renderable);
  fl_subsurface_swap_buffers(self->render_area);
}
...
static void fl_renderable_iface_init(FlRenderableInterface* iface) {
  ...
  iface->swap_buffers = fl_view_swap_buffers;
}
...
static void setup_engine(FlView* self) {
  // FIXME: Handle error
  fl_opengl_manager_create_contexts(fl_engine_get_opengl_manager(self->engine),
                                    nullptr);
